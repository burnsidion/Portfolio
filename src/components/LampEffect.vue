<template>
  <div :class="
    cn(
      'relative z-0 flex w-full flex-col items-center justify-start overflow-hidden rounded-md bg-slate-950',
      $props.class
    )
  ">
    <div class="relative isolate z-0 flex w-full items-start justify-center">
      <!-- Conic Gradient -->
      <div :style="{
          backgroundImage: `conic-gradient(var(--conic-position), var(--tw-gradient-stops))`,
        }"
        class="animate-conic-gradient bg-gradient-conic absolute inset-auto right-1/2 h-56 w-60 overflow-hidden from-cyan-500 via-transparent to-transparent text-white opacity-50 [--conic-position:from_70deg_at_center_top]">
        <div
      ...
      <!-- Spotlight -->
      <div
        class="animate-spotlight absolute inset-auto z-30 h-36 w-32 max-w-[80%] sm:max-w-[60%] -translate-y-16 sm:-translate-y-24 rounded-full bg-cyan-400 blur-2xl">
      </div>

      <!-- Glowing Line -->
      <div class="animate-glowing-line absolute inset-auto z-50 h-0.5 w-[80%] sm:w-60 -translate-y-28 bg-cyan-400"></div>

      <div class="absolute inset-auto z-40 h-44 w-full translate-y-[-12.5rem] max-w-[90%] sm:max-w-full bg-slate-950"></div>
    </div>

    <div class="relative z-50 flex translate-y-4 flex-col items-center px-5">
      <slot />
    </div>
  </div>
</template>
