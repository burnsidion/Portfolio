<template>
    <footer class="w-full">
        <div class="flex justify-center sm:justify-start items-center gap-5 mt-4 md:gap-8 lg:gap-10 sm:mx-auto">
            <a href="https://github.com/burnsidion" target="_blank"
                class="opacity-70 hover:opacity-100 transition-transform hover:scale-110 filter invert">
                <font-awesome-icon :icon="['fab', 'github']" class="text-2xl" />
            </a>
            <a href="https://www.linkedin.com/in/ian-burnside/" target="_blank"
                class="opacity-70 hover:opacity-100 transition-transform hover:scale-110 filter invert">
                <font-awesome-icon :icon="['fab', 'linkedin']" class="text-2xl" />
            </a>
            <a href="https://www.instagram.com/burnsidion" target="_blank"
                class="opacity-70 hover:opacity-100 transition-transform hover:scale-110 filter invert">
                <font-awesome-icon :icon="['fab', 'instagram']" class="text-2xl" />
            </a>
        </div>
    </footer>
</template>
